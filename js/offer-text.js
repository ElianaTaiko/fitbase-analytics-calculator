...
function buildOfferHtml(result, config, validUntilStr) {
  ...
  lines.push("");
  lines.push(`Тариф: ${bold(result.tariff.label)}`);
  ...
  if (result.family === "franchise") {
    ...
    const includedParts = [];
    if (result.includedDashboards.length > 0) {
      includedParts.push(`дашборды — ${result.includedDashboards.map((d) => bold(d.label)).join(", ")}`);
    }
  ...
  if (result.installTotal > 0 && result.installBreakdown.length === 1) {
    ...
    lines.push(
      `Разовый платёж (Install): ${bold(money(result.installTotal, config.currency))} — ${esc(
        result.installBreakdown[0].label
      )}`
    );
  } else if (result.installTotal > 0) {
    lines.push(`Разовый платёж (Install): ${bold(money(result.installTotal, config.currency))}`);
    result.installBreakdown.forEach((row) => {
      lines.push(`  — ${esc(row.label)}: ${bold(money(row.amount, config.currency))}`);
    });
